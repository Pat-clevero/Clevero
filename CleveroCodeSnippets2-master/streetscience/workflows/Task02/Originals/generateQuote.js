async function script(C) {
    ...
    const [job, jobAsIV] = await Promise.all([
        C.getCurrentEntry({
            ...
            subrecords: [
                {
                    internalId: "street-science-classroom-kits",
                },
            ],
    ...
    const classroomKitsIds = job.subrecords["street-science-classroom-kits"]
        .map((kit) => kit["2708638-item"] && kit["2708638-item"][0])
        .filter((id) => id);
    ...
    let sessionOutline = "";
    // Generate Session over View
    sessions.forEach((session, index) => {
        sessionOutline +=
            session["2708638-group-name"] +
            " - " +
            moment(session["2708638-start-time"])
                .tz("Australia/Brisbane")
                .format("LT") +
            " - " +
            moment(session["2708638-end-time"])
                .tz("Australia/Brisbane")
                .format("LT") +
            "<br>";
    });
    ...
    const quoteToBeGenerated = {
        ...
        "2708638-job-date": moment(job["2708638-start-time"]).format("YYYY-MM-DD"),
        "date-created": moment().tz("Australia/Brisbane").format("YYYY-MM-DD"),
        "sales-rep": loggedInUserId ? [loggedInUserId] : [],
        "2708638-session-outline": sessionOutline,
    };
    ...
    sessions.forEach((session, index) => {
        const quantity = +session["2708638-no-of-kids"] || 1;
        const rate = +session["2708638-rate-per-head"] || 0;
        const applyDiscount = session["2708638-apply-discount"] || false;
        ...
        if (session["2708638-start-time"]) {
            startTime = moment(session["2708638-start-time"])
                .tz(companyTimeZone)
                .format("YYYY-MM-DD hh:mm a");
        }
        if (session["2708638-end-time"]) {
            endTime = moment(session["2708638-end-time"])
                .tz(companyTimeZone)
                .format("YYYY-MM-DD hh:mm a");
        }
        ...
        const description =
            startTime && endTime
                ? `${session["2708638-invoice-line-description"]} ${startTime}-${endTime}`
                : `${session["2708638-invoice-line-description"]}`;

        serviceLineItems.push({
            parent: quoteId,
            index: index + 1,
            item:
                session["2708638-xero-item"] &&
                session["2708638-xero-item"].length
                    ? [session["2708638-xero-item"][0]]
                    : [],
            description,
            quantity,
            rate,
    ...
    for (const kit of job.subrecords["street-science-classroom-kits"]) {
        const quantity = +kit["2708638-quantity"] || 1;
        const rate = +kit["2708638-rate"] || 0;

        const item =
            classroomKits.find(
                (ckit) => +ckit.recordValueId === kit["2708638-item"][0]
            ) || {};
