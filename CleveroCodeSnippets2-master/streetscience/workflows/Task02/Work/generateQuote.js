async function script(C) {
    ...
    /**
     * IF same item with same discount and same rate ==> group them as one item, add the quantities together and just display one item (representing all of them)
     */
    ...
    for (let key in groupedSessions) {
        if (groupedSessions.hasOwnProperty(key)) {
            ...
            // push just one item to serviceLineItems
            ...
            const description =
                startTime && endTime
                    ? `${group[0]["2708638-invoice-line-description"]} ${startTime}-${endTime}`
                    : `${group[0]["2708638-invoice-line-description"]}`;
