async function script(C) {
    ...
    const totalChargeBeforeTravelFee = serviceLineItems.reduce((agg, value) => {
        return agg + value.net;
    }, 0);
    C.log({ totalChargeBeforeTravelFee });
    ...
    const categoryMinimumCharge = +eventCategoryResponse["2708638-minimum-charge"] || 0;
    const valueToCharge = Math.max(categoryMinimumCharge, totalChargeBeforeTravelFee);

    const minimumCharge = 1200;
    if (valueToCharge < minimumCharge) {
        const difference = minimumCharge - valueToCharge;
        serviceLineItems.push({
            parent: quoteId,
            index: serviceLineItems.length + 1,
