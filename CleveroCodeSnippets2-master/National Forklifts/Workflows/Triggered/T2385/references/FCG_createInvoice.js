async function script(C) {
    ...
    const createOrderLineItems = async (parentInvoiceId, entries) => {
        let arrayOfArraysOfOrderLineValues = await Promise.all(
            entries.map(async (currentEntry) => {
                let primaryServiceFieldFound = currentEntry.hasOwnProperty(
                    "1795685-primary-service"
                );
                let secondaryServiceFieldFound = currentEntry.hasOwnProperty(
                    "1795685-secondary-services"
                );
                ...
                if (
                    primaryServiceFieldFound == true &&
                    currentEntry["1795685-primary-service"] &&
                    currentEntry["1795685-primary-service"].length > 0
                ) {
                    // C.log("PRIMARY EXECUTED***");
                    let primaryItemSelected =
                    ...
                    let rateValue = primaryItemSelectedDetails[
                        "sales-details-unit-price"
                    ]
                        ? primaryItemSelectedDetails["sales-details-unit-price"]
                        : 0;
                    let taxValue = primaryItemSelectedDetails[
                        "sales-details-tax-type"
                    ][0]
                        ? primaryItemSelectedDetails[
                              "sales-details-tax-type"
                          ][0]
                        : 0;
                    ...
                    let taxNumeric = taxDetails["effective-tax-rate"]
                        ? taxDetails["effective-tax-rate"]
                        : 0;
                    ...
                    orderLineValues.push({
                        ...
                        description: primaryItemSelectedDetails.description
                            ? `${
                                  primaryItemSelectedDetails.description
                              } - ${moment(currentEntry["1795685-start-time"])
                                  .tz("Australia/Sydney")
                                  .format("DD/MM/YYYY")}`
                            : "",
                        rate: rateValue,
                        "tax-rate": primaryItemSelectedDetails[
                            "sales-details-tax-type"
                        ]
                            ? primaryItemSelectedDetails[
                                  "sales-details-tax-type"
                            ...
                            : [],
                        account: primaryItemSelectedDetails[
                            "sales-details-account"
                        ]
                            ? primaryItemSelectedDetails[
                                  "sales-details-account"
                              ]
                ...
                if (
                    secondaryServiceFieldFound == true &&
                    currentEntry["1795685-secondary-services"] &&
                    currentEntry["1795685-secondary-services"].length > 0
                ) {
                    let secondaryItemSelectedIds =
                        currentEntry["1795685-secondary-services"];

                    for (let i = 0; i < secondaryItemSelectedIds.length; i++) {
                        let currentEntryEntryId = secondaryItemSelectedIds[i];

                        let secondaryItemSelectedDetails = await C.getEntry({
                            recordInternalId: "xero-items",
                            entryId: currentEntryEntryId,
                        });
                        let rateValue = secondaryItemSelectedDetails[
                            "sales-details-unit-price"
                        ]
                            ? secondaryItemSelectedDetails[
                                  "sales-details-unit-price"
                              ]
                            : 0;
                        let taxValue = secondaryItemSelectedDetails[
                            "sales-details-tax-type"
                        ][0]
                            ? secondaryItemSelectedDetails[
                                  "sales-details-tax-type"
                        ...
                        let taxNumeric = taxDetailsSecondary[
                            "effective-tax-rate"
                        ]
                            ? taxDetailsSecondary["effective-tax-rate"]
                            : 0;
                        let numericTaxValueFinal =
                            (rateValue * taxNumeric) / 100;
                        let netValueFinal = rateValue;
                        let totalValue = numericTaxValueFinal + netValueFinal;
                        orderLineValues.push({
                            item: [secondaryItemSelectedIds[i]],
                            description: secondaryItemSelectedDetails.description
                                ? `${
                                      secondaryItemSelectedDetails.description
                                  } - ${moment(
                                      currentEntry["1795685-start-time"]
                                  )
                                      .tz("Australia/Sydney")
                                      .format("DD/MM/YYYY")}`
                                : "",
                            rate: rateValue,
                            "tax-rate": secondaryItemSelectedDetails[
                                "sales-details-tax-type"
                            ]
                                ? secondaryItemSelectedDetails[
                                      "sales-details-tax-type"
                                ...
                                : [],
                            account: secondaryItemSelectedDetails[
                                "sales-details-account"
                            ]
                                ? secondaryItemSelectedDetails[
                                      "sales-details-account"
                ...
                if (
                    currentEntry["1795685-discount"] && currentEntry["1795685-discount"] > 0
                ) {
                    let discount =
                        currentEntry["1795685-discount"];
        ...
        const values = arrayOfArraysOfOrderLineValues.flatMap(
            (innerArray) => innerArray
        );
        C.addJsonToSummary({ LineOrderItems: values });
        ...
        C.log("order line items are created successfully");
    ...
    let invoiceValues = [];
    let invoicePayer = currentEntry["1795685-law-firm"];
    let invoicePayerType = currentEntry["1795685-invoice-payer"];
    if (invoicePayerType && invoicePayerType[0] == "2258618") {
        invoicePayer = currentEntry["1795685-law-firm"];
    } else {
        invoicePayer = currentEntry["1795685-client"];
    }

    const clientResponse = await C.getEntry({
        recordInternalId: "standard-organisations",
        entryIds: currentEntry["1795685-client"],
    });
    const clientName = clientResponse[0].name;

    invoiceValues.push({
        "1795685-appointment": [+currentEntry.recordValueId],
        reference: clientName,
        "1795685-linked-client": currentEntry["1795685-client"],
        customer: invoicePayer,
        date: moment().format("YYYY-MM-DD"),
        "due-date": moment().add(14, "days").format("YYYY-MM-DD"),
        status: [34126],
    ...
    const childrenAppointments = await getChildrenAppointments(
        currentAppointment.recordValueId
    );
    childrenAppointments.entries.forEach((appointment) => {
        entriesArray.push(appointment);
    });

    let createdInvoiceWithOrderLineItems = await createOrderLineItems(
    ...
    await C.updateEntries({
        updates: [
            {
                value: {
                    "1795685-invoice-generated": true,
                    "1795685-invoice-date": C.moment().format("YYYY-MM-DD"),
                },
                recordInternalId: "ferrari-consulting-group-appointments",
    ...
    return C.addRedirect(
        `/app/records/1719816/view/${createInvoice.success[0].id}`
    );
