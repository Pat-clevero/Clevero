async function script(C) {
    ...
    if (itemCode && itemCode.length > 0) {
        ...
        if (
            ...
            let tgaStatus;
            const tgaDocumentStatus = {
                approved: 1734656,
                sent_to_tga: 1734655,
            };
            const tgaApprovalStatus = {
                tga_approved: 1678997,
                tga_applied: 200228777,
            };
            if(tgaDocuments.entries[0]["1614495-status"] == tgaDocumentStatus.approved)
                tgaStatus = [tgaApprovalStatus.tga_approved];
            else if(tgaDocuments.entries[0]["1614495-status"] == tgaDocumentStatus.sent_to_tga)
                tgaStatus = [tgaApprovalStatus.tga_applied];
            
            if(tgaDocuments.entries && tgaDocuments.entries.length>0){
                treatmentPlanEntry["1614495-tga-approval-status"] = tgaStatus;
