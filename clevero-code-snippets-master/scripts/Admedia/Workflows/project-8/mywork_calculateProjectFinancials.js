async function script(C) {
    ...
    if (
        eventOriginatorInternalId === "admedia-timesheets" &&
        eventType !== "AFTER_DELETE"
    ) {
        const autoId = currentEntry.autoId;

        const task = currentEntry["1662670-task"][0];
        const taskObject = await C.getEntry({
            entryId: task,
            recordInternalId: "admedia-tasks",
        });
        const taskTitle = taskObject["1662670-title"];
        ...
        const updateTimesheetResult = await C.updateEntries({
            updates: [
                {
                    value: {
                        "1662670-revenue": +timesheetRevenue,
                        "1662670-cost": +timesheetCost,
                        "1662670-name": autoId + " - " + taskTitle + " - " + employeeName,
                    },
        ...
        C.addJsonToSummary({ updateTimesheetResult });
        C.log("Timesheet Cost and Revenue calculated before project totals");
    }
