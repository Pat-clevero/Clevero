async function script(C) {
    ...
    const totalNetCharge = serviceLineItems.reduce((agg, value) => {
        return agg + value.net;
    }, 0);
    C.log({ totalNetCharge });
    let eventCategoryId = job["2708638-secondary-job-type"][0];
    ...
    const categoryMinimumCharge = +eventCategoryResponse["2708638-minimum-charge"];
    if (totalNetCharge < categoryMinimumCharge && categoryMinimumCharge > 0) {
        const difference = categoryMinimumCharge - totalNetCharge;
        serviceLineItems.push({
            parent: quoteId,
            index: serviceLineItems.length + 1,
            item: [300289349], // Service fee for minimum charge,
            description: "",
            quantity: 1,
            rate: difference,
            "tax-rate": [300101130], //GST on income
        ...
        C.log(`Service fee for minimum charge ${difference}`);
    }
    ...
    if (+job["2708638-travel-fee"] > 0) {
        ...
        serviceLineItems.push({
            parent: quoteId,
            index: serviceLineItems.length + 1,
            item: [300510142], // Travel Surcharge,
            description:
    ...
    const totalChargeForDeposit = serviceLineItems.reduce((agg, value) => {
        return agg + value.total;
    }, 0);
    const deposit = totalChargeForDeposit * 0.3;
